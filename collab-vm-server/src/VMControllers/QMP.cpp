...
void QMP::TimerCallback(const asio::error_code& ec)
{
	if (ec || stopped_)
		return;
...
void QMP::OnResolve(const asio::error_code& ec, tcp::resolver::iterator iterator)
{
	if (stopped_)
		return;
...
void QMP::OnConnect(const asio::error_code& ec, tcp::resolver::iterator iterator)
{
	if (stopped_)
		return;
...
void QMP::OnReadLine(const asio::error_code& ec, size_t size)
{
	if (stopped_)
		return;
	...
	if (!ec)
	{
		...
		if (size > 2)
		{
			...
			memcpy(buf, asio::buffer_cast<const char*>(buf_.data()), size - 2);
			...
			case kResponse:
				...
				else
				{
					...
					cout << "QMP invalid handshake response: " <<
						std::string(asio::buffer_cast<const char*>(buf_.data()), size - 2) << endl;
					Disconnect();
				}
				break;
			case kFinished:
...
void QMP::OnWrite(const asio::error_code& ec, size_t size)
{
	if (stopped_)
		return;
...
{
	async_write(socket_, asio::buffer(data, len),
