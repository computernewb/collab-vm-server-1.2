...
protected:
	VMController(CollabVMServer& server, asio::io_service& service, const std::shared_ptr<VMSettings>& settings);

	virtual void OnAddUser(CollabVMUser& user) = 0;

	virtual void OnRemoveUser(CollabVMUser& user) = 0;

	void InitAgent(const VMSettings& settings, asio::io_service& service);
	 ...
	 */
	asio::steady_timer agent_timer_;
...
private:
	...
	void VoteEndedCallback(const asio::error_code& ec);

	void TurnTimerCallback(const asio::error_code& ec);

	asio::steady_timer turn_timer_;
