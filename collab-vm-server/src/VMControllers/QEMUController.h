...
public:
	 ...
	 */
	QEMUController(CollabVMServer& server, asio::io_service& service, const std::shared_ptr<VMSettings>& settings);
...
private:
	...
	*/
	...
	void StartQMPCallback(const asio::error_code& ec);
	...
	void StartGuacClientCallback(const asio::error_code& ec);

	void ProcessKillTimeout(const asio::error_code& ec);
	 ...
	 */
	void HandleChildSignal(const asio::error_code&, int signal);
