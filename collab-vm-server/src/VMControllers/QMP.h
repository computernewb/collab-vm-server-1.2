...
public:
	...
	QMP(asio::io_service& service, const std::string& host, uint16_t port);
...
private:
	...
	void OnResolve(const asio::error_code& ec, asio::ip::tcp::resolver::iterator iterator);
	void StartConnection(asio::ip::tcp::resolver::iterator endpoint_iter);
	void OnConnect(const asio::error_code& ec, asio::ip::tcp::resolver::iterator iterator);
	void OnReadLine(const asio::error_code& e, std::size_t size);
	void OnWrite(const asio::error_code& ec, size_t size);
	void ReadLine();
