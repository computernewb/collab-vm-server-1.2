...
static asio::io_service::work* qmp_work_;
static asio::io_service* qmp_service_;

QEMUController::QEMUController(CollabVMServer& server, asio::io_service& service, const std::shared_ptr<VMSettings>& settings) :
...
#endif
{
	...
	if (!qmp_count_++)
	{
		qmp_service_ = new asio::io_service();
		qmp_work_ = new asio::io_service::work(*qmp_service_);
		qmp_thread_ = std::thread([](asio::io_service* service)
		{
			service->run();
			delete service;
		}, qmp_service_);
...
#ifndef _WIN32
void QEMUController::HandleChildSignal(const asio::error_code& ec, int signal)
{
	if (ec)
		return;
	int status = 0;
...
void QEMUController::InitQMP()
{
	...
	else if (settings_->QMPSocketType == VMSettings::SocketType::kLocal)
	{
		...
		{
#ifdef _WIN32
			qmp_address_ = R"(\\.\pipe\collab-vm-qmp-)";
#else
...
void QEMUController::StopQEMU()
{
	...
	{
#ifndef _WIN32
		// Wait up to 5 seconds for the QEMU process to terminate
		asio::error_code ec;
		timer_.expires_from_now(std::chrono::seconds(5), ec);
		timer_.async_wait(std::bind(&QEMUController::ProcessKillTimeout,
...
void QEMUController::StartQMPCallback(const asio::error_code& ec)
{
...
void QEMUController::StartGuacClientCallback(const asio::error_code& ec)
{
...
void QEMUController::ProcessKillTimeout(const asio::error_code& ec)
{
	if (ec)
		return;
...
void QEMUController::GuacDisconnect()
{
	...
	else if (internal_state_ == InternalState::kConnected)
	{
		...
		{
			std::cout << "Gaucamole client unexpectedly disconnected (Code: " <<
...
void QEMUController::OnAgentDisconnect(bool protocol_error)
{
		 ...
		 internal_state_ == InternalState::kConnected) && !protocol_error)
	{
		...
		agent_timer_.async_wait([this, self](const asio::error_code& ec)
		{
			if (!ec && (internal_state_ == InternalState::kVNCConnecting |
						internal_state_ == InternalState::kConnected))
				agent_->Connect(std::weak_ptr<AgentCallback>(std::static_pointer_cast<AgentCallback>(shared_from_this())));
