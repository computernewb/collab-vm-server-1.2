...
{
public:
	CollabVMServer(asio::io_service& service);
...
private:
	...
	protected:
		void ValidateCallback(websocketpp::connection_hdl handle, const std::string& buf) override
		{
			...
			const asio::ip::tcp::endpoint& ep = con->get_raw_socket().remote_endpoint(asio_ec);
			if (asio_ec)
				return;
	...
	void TimerCallback(const asio::error_code& ec, ActionType action);
	void VMPreviewTimerCallback(const asio::error_code ec);
	void IPDataTimerCallback(const asio::error_code& ec);
	...
	void OnUploadTimeout(const asio::error_code ec, std::shared_ptr<UploadInfo> upload_info);
	...
	bool FindIPv4Data(const asio::ip::address_v4& addr, IPData*& ip_data);
	bool FindIPData(const asio::ip::address& addr, IPData*& ip_data);
	IPData* CreateIPv4Data(const asio::ip::address_v4& addr, bool one_connection);
	IPData* CreateIPData(const asio::ip::address& addr, bool one_connection);
	 ...
	 */
	asio::steady_timer vm_preview_timer_;
	 ...
	 */
	asio::steady_timer ip_data_timer;
	 ...
	 * are stopped.
	 */
	std::unique_ptr<asio::io_service::work> asio_work_;
