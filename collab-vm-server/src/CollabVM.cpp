...
CollabVMServer::CollabVMServer(asio::io_service& service) :
	service_(service),
	stopping_(false),
...
void CollabVMServer::Run(uint16_t port, string doc_root)
{
	...
	server_.set_http_partial_handler(bind(&CollabVMServer::OnHttpPartial, shared_from_this(),
	...
	asio_work_ = std::unique_ptr<asio::io_service::work>(new asio::io_service::work(service_));
	...
	process_thread_ = thread(bind(&CollabVMServer::ProcessingThread, shared_from_this()));
...
void CollabVMServer::OnHttp(connection_hdl handle)
{
	...
	unique_lock<mutex> lock(http_connections_lock_);
...
bool CollabVMServer::FindIPv4Data(const asio::ip::address_v4& addr, IPData*& ip_data)
{
...
bool CollabVMServer::FindIPData(const asio::ip::address& addr, IPData*& ip_data)
{
	if (addr.is_v4())
	{
	...
	{
		const asio::ip::address_v6& addr_v6 = addr.to_v6();
...
IPData* CollabVMServer::CreateIPv4Data(const asio::ip::address_v4& addr, bool one_connection)
{
...
IPData* CollabVMServer::CreateIPData(const asio::ip::address& addr, bool one_connection)
{
	if (addr.is_v4())
	{
...
#define UPLOAD_PREFIX "/upload?"
										...
										GetBodyContentType(contentType) == HttpContentType::kOctetStream)
		{
			...
			unique_lock<mutex> lock(upload_lock_);
			...
			if (it != upload_ids_.end())
			{
				...
				asio::steady_timer* timer = upload_info->timeout_timer;
...
bool CollabVMServer::OnValidate(connection_hdl handle)
{
	...
	for (auto it = protocols.begin(); it != protocols.end(); it++)
	{
		if (*it == "guacamole")
		{
			...
			asio::error_code ec;
			const asio::ip::tcp::endpoint& ep = con->get_raw_socket().remote_endpoint(ec);
			if (ec)
				return false;

			const asio::ip::address& addr = ep.address();

			unique_lock<mutex> ip_lock(ip_lock_);
			IPData* ip_data;
...
void CollabVMServer::OnOpen(connection_hdl handle)
{
	...
	unique_lock<mutex> lock(process_queue_lock_);
...
void CollabVMServer::TimerCallback(const asio::error_code& ec, ActionType action)
{
	if (ec)
		return;
...
void CollabVMServer::VMPreviewTimerCallback(const asio::error_code ec)
{
	if (ec)
		return;
...
void CollabVMServer::IPDataTimerCallback(const asio::error_code& ec)
{
	if (ec)
		return;

	std::lock_guard<mutex> lock(ip_lock_);
...
void CollabVMServer::ProcessingThread()
{
	while (true)
	{
		...
		case ActionType::kHttpUploadFinished:
		{
			...
			if (asio::steady_timer* timer = upload_info->timeout_timer)
			{
	...
	case ClientFileOp::kBegin:
		if (args.size() == 4 && !user->upload_info && !user->ip_data.upload_in_progress)
		{
			...
			if (user->vm_controller->IsFileUploadValid(user, filename, file_size, run_file))
			{
				...
				std::unique_lock<mutex> lock(upload_lock_);
...
void CollabVMServer::OnUploadTimeout(const asio::error_code ec, std::shared_ptr<UploadInfo> upload_info)
{
	if (ec)
		return;
...
void CollabVMServer::StartFileUpload(CollabVMUser& user)
{
	...
	if (upload_info->file_stream.is_open() && upload_info->file_stream.good())
	{
		...
		asio::steady_timer* timer = new asio::steady_timer(service_);
										...
										const std::shared_ptr<CollabVMUser>* user, FileUploadResult result)
{
	...
	else
	{
		...
		// locking is required
		unique_lock<mutex> lock(ip_lock_);
		uint32_t cooldown_time = vm_controller.GetSettings().UploadCooldownTime;
		SetUploadCooldownTime(upload_info->ip_data, cooldown_time);
		if (upload_info->ip_data.connections > 1)
		{
...
void CollabVMServer::CancelFileUpload(CollabVMUser& user)
{
	...
	if (prev_state == UploadInfo::HttpUploadState::kNotStarted)
	{
		std::lock_guard<mutex> lock(upload_lock_);
		if (user.upload_info->upload_it != upload_ids_.end())
			upload_ids_.erase(user.upload_info->upload_it);
	}

	if (asio::steady_timer* timer = user.upload_info->timeout_timer)
	{
...
void CollabVMServer::WriteServerSettings(rapidjson::Writer<rapidjson::StringBuffer>& writer)
{
	...
	writer.String(server_settings_[kChatRateCount]);
	writer.Uint(database_.Configuration.ChatRateCount);

	writer.String(server_settings_[kChatRateTime]);
	writer.Uint(database_.Configuration.ChatRateTime);

	writer.String(server_settings_[kChatMuteTime]);
	writer.Uint(database_.Configuration.ChatMuteTime);

	writer.String(server_settings_[kMaxConnections]);
	writer.Uint(database_.Configuration.MaxConnections);

	writer.String(server_settings_[kMaxUploadTime]);
	...
	for (auto it = database_.VirtualMachines.begin(); it != database_.VirtualMachines.end(); it++)
	{
		...
		writer.StartObject();
		writer.String("name");
		writer.String(vm->Name);
		for (size_t i = 0; i < sizeof(vm_settings_)/sizeof(std::string); i++)
		{
			switch (i)
			{
			case kAutoStart:
				writer.String(vm_settings_[kAutoStart]);
				writer.Bool(vm->AutoStart);
				break;
			case kDisplayName:
				writer.String(vm_settings_[kDisplayName]);
				writer.String(vm->DisplayName);
				break;
			case kHypervisor:
				writer.String(vm_settings_[kHypervisor]);
				writer.String(hypervisor_names_[static_cast<size_t>(vm->Hypervisor)]);
				break;
			case kStatus:
			{
				writer.String(vm_settings_[kStatus]);
				auto vm_it = vm_controllers_.find(vm->Name);
				if (vm_it != vm_controllers_.end())
				{
			...
			case kRestoreShutdown:
				writer.String(vm_settings_[kRestoreShutdown]);
				writer.Bool(vm->RestoreOnShutdown);
				break;
			case kRestoreTimeout:
				writer.String(vm_settings_[kRestoreTimeout]);
				writer.Bool(vm->RestoreOnTimeout);
				break;
			case kVNCAddress:
				writer.String(vm_settings_[kVNCAddress]);
				writer.String(vm->VNCAddress);
				break;
			case kVNCPort:
				writer.String(vm_settings_[kVNCPort]);
				writer.Uint(vm->VNCPort);
				break;
			case kQMPSocketType:
				writer.String(vm_settings_[kQMPSocketType]);
				writer.String(vm->QMPSocketType == VMSettings::SocketType::kLocal ? "local" : "tcp");
				break;
			case kQMPAddress:
				writer.String(vm_settings_[kQMPAddress]);
				writer.String(vm->QMPAddress);
				break;
			case kQMPPort:
				writer.String(vm_settings_[kQMPPort]);
				writer.Uint(vm->QMPPort);
				break;
			case kQEMUCmd:
				writer.String(vm_settings_[kQEMUCmd]);
				writer.String(vm->QEMUCmd);
				break;
			case kQEMUSnapshotMode:
				writer.String(vm_settings_[kQEMUSnapshotMode]);
				writer.String(snapshot_modes_[static_cast<size_t>(vm->QEMUSnapshotMode)]);
				break;
			case kTurnsEnabled:
				writer.String(vm_settings_[kTurnsEnabled]);
				writer.Bool(vm->TurnsEnabled);
				break;
			case kTurnTime:
				writer.String(vm_settings_[kTurnTime]);
				writer.Uint(vm->TurnTime);
				break;
			case kVotesEnabled:
				writer.String(vm_settings_[kVotesEnabled]);
				writer.Bool(vm->VotesEnabled);
				break;
			case kVoteTime:
				writer.String(vm_settings_[kVoteTime]);
				writer.Uint(vm->VoteTime);
				break;
			case kVoteCooldownTime:
				writer.String(vm_settings_[kVoteCooldownTime]);
				writer.Uint(vm->VoteCooldownTime);
				break;
			case kAgentEnabled:
				writer.String(vm_settings_[kAgentEnabled]);
				writer.Bool(vm->AgentEnabled);
				break;
			case kAgentSocketType:
				writer.String(vm_settings_[kAgentSocketType]);
				writer.String(vm->AgentSocketType == VMSettings::SocketType::kLocal ? "local" : "tcp");
				break;
			case kAgentUseVirtio:
				writer.String(vm_settings_[kAgentUseVirtio]);
				writer.Bool(vm->AgentUseVirtio);
				break;
			case kAgentAddress:
				writer.String(vm_settings_[kAgentAddress]);
				writer.String(vm->AgentAddress);
				break;
			case kAgentPort:
				writer.String(vm_settings_[kAgentPort]);
				writer.Uint(vm->AgentPort);
				break;
			case kRestoreHeartbeat:
				writer.String(vm_settings_[kRestoreHeartbeat]);
				writer.Bool(vm->RestoreHeartbeat);
				break;
			case kHeartbeatTimeout:
				writer.String(vm_settings_[kHeartbeatTimeout]);
				writer.Uint(vm->HeartbeatTimeout);
				break;
			case kUploadsEnabled:
				writer.String(vm_settings_[kUploadsEnabled]);
				writer.Bool(vm->UploadsEnabled);
				break;
			case kUploadCooldownTime:
				writer.String(vm_settings_[kUploadCooldownTime]);
				writer.Uint(vm->UploadCooldownTime);
				break;
			case kUploadMaxSize:
				writer.String(vm_settings_[kUploadMaxSize]);
				writer.Uint(vm->MaxUploadSize);
				break;
			case kUploadMaxFilename:
				writer.String(vm_settings_[kUploadMaxFilename]);
