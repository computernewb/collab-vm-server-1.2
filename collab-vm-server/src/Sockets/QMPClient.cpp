...
void QMPClient::OnReadLine(const asio::error_code& ec, size_t size, std::shared_ptr<SocketCtx>& ctx)
{
	if (ctx->IsStopped())
		return;
	...
	if (size > 2)
	{
		...
		std::memcpy(buf, asio::buffer_cast<const char*>(buf_.data()), size - 2);
		...
		case ConnectionState::kCapabilities:
		{
			...
			else
			{
				...
				std::cout << "QMP invalid capabilities command: " <<
					std::string(asio::buffer_cast<const char*>(buf_.data()), size - 2) << std::endl;
				DisconnectSocket();
			}
			break;
		}
...
void QMPClient::OnWrite(const asio::error_code& ec, size_t size, std::shared_ptr<SocketCtx> ctx)
{
	if (ctx->IsStopped())
		return;
