...
{
public:
	QMPClient(asio::io_service& service) :
		timer_(service),
...
protected:
	...
	{
		asio::async_read_until(socket, buf_, "\r\n",
...
private:
	...
	void OnReadLine(const asio::error_code& ec, size_t size, std::shared_ptr<SocketCtx>& ctx);
...
{
public:
	QMPClientSocket(asio::io_service& service, const std::string& host, uint16_t port)
		: TCPSocketClient<QMPClient>(service, host, port)
	{
	}

	QMPClientSocket(asio::io_service& service, const std::string& name) :
